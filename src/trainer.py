...
from models.recommenders import ReCRec_I,ReCRec_D,ReCRec_F
...
class Trainer:
    ...
    def run(self) -> None:
        ...
        exposure = np.load(f'../data/{self.dataset_name}/point/exposure.npy',allow_pickle=True)
        vad = np.load(f'../data/{self.dataset_name}/point/val.npy',allow_pickle=True)
        test = np.load(f'../data/{self.dataset_name}/point/test.npy',allow_pickle=True)
        pscore = np.load(f'../data/{self.dataset_name}/point/pscore.npy',allow_pickle=True)
        ...
        print('test result:')
        ret = aoa_evaluator( user_embed=best_uemb,item_embed=best_iemb, model_name=self.model_name, test=test, at_k=[5],item_bias=best_ib)
